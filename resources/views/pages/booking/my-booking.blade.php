...
@section('content')
        ...
        <div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white">
            <div class="flex gap-4">
                <div class="flex w-[120px] h-[132px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                    <img src="{{ asset('storage/' . $order->boardingHouse->thumbnail) }}" class="w-full h-full object-cover"
                        alt="icon">
                </div>
                <div class="flex flex-col gap-3 w-full">
                    <p class="font-semibold text-lg leading-[27px] line-clamp-2 min-h-[54px]">
                        {{ $order->boardingHouse->name }}</p>
    ...
    <div
        ...
        <div class="flex flex-col gap-4 pt-[22px]">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('assets/images/icons/calendar.svg') }}" class="w-6 h-6 flex shrink-0"
                        alt="icon">
                    <p class="text-ngekos-grey">Booking Code</p>
                </div>
            ...
            <div class="flex items-center justify-between">
                ...
                <p class="font-semibold">{{ $order->duration }} Months</p>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('assets/images/icons/calendar.svg') }}" class="w-6 h-6 flex shrink-0"
                ...
                <p class="font-semibold">
                    {{ \Carbon\Carbon::parse($order->start_date)->translatedFormat('j F Y') }}</p>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('assets/images/icons/calendar.svg') }}" class="w-6 h-6 flex shrink-0"
                ...
                <p class="font-semibold">
                    {{ \Carbon\Carbon::parse($order->start_date)->addMonths($order->duration)->translatedFormat('j F Y') }}
                </p>
            </div>
        </div>
    </div>
    <div
        ...
        <div class="flex flex-col gap-4 pt-[22px]">
            <div class="flex items-center justify-between">
                ...
                @if ($order->payment_method == 'full_payment')
            ...
            <div class="flex items-center justify-between">
                ...
                <p class="font-semibold">Rp {{ number_format($order->room->price_per_month, thousands_separator: '.') }} x
                    {{ $order->duration }}
                </p>
            </div>
            <div class="flex items-center justify-between">
                ...
                <p class="font-semibold">Rp.
                    {{ number_format($order->room->price_per_month * $order->duration, thousands_separator: '.') }}</p>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                ...
                <p class="font-semibold">Rp.
                    {{ number_format($order->room->price_per_month * $order->duration * 0.11, thousands_separator: '.') }}
                </p>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                ...
                <p class="font-semibold">Rp.
                    {{ number_format($order->room->price_per_month * $order->duration * 0.01, thousands_separator: '.') }}
                </p>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                ...
                <p class="font-semibold">Rp. {{ number_format($order->total_amount, thousands_separator: '.') }}</p>
            </div>
            ...
            @else
                <div class="flex items-center justify-between">
                    ...
                    <p class="rounded-full p-[6px_12px] bg-[#91BF77] font-bold text-xs leading-[18px]">PAID</p>
                </div>
            @endif
        </div>
    </div>
    <div id="BottomButton" class="relative flex w-full h-[98px] shrink-0">
        <div class="fixed bottom-[30px] w-full max-w-[640px] px-5 z-10">
            <a href="https://wa.me/6289650557420?text=Hallo%2C%20saya%20{{ $order->name }}%20ingin%20konfirmasi%20terkait%20order%20booking%20saya%20dengan%20code%20{{ $order->code }}"
